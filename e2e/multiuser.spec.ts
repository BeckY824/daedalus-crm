...
test.describe("D 组：批量操作交叉", () => {
  test("两人同时批量改状态，都要收到真实条数且不互相锁死", async ({ browser }) => {
    ...
    /*
      两个状态随便挑，但**不能挑「已试听」「与家人商议」「已决定报名」**这三个。

      它们的存储值天生带教培味，默认业务配置给了另一套显示名
      （DEFAULT_BUSINESS.statusLabels：已试听 → 已演示）。菜单上写的是显示名，
      按存储值去点，在一个没存过业务配置的库上根本找不到。
      这条用例原先写的就是「已试听」，能过完全是搭了顺风车：business-settings
      那一组排在前面，它把显示名改回去又保存了一次，于是服务端那张设置缓存里
      留着「已试听」——而它的 afterAll 只删了库里的行，没动缓存。
      在那之后任何一次设置写入（比如别的用例配一下 AI）都会让缓存重新读库，
      读到的是「没有业务配置」，菜单立刻变回「已演示」，这一条就挂了。
      单跑这一条（-g）也一直是挂的，只是没人单跑过。
    */
