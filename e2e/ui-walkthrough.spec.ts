...
/**
 * 这一组比别的用例宽三分之一的超时（默认 60 秒）。
 *
 * 不是因为它们慢得不正常，而是它们本来就干得多：三条溢出检查各要走遍 12 个页面，
 * 每页导航一次再存一张整页截图，光截图就十二张。实测 42–59 秒，**贴着默认上限**——
 * 机器一忙就翻车，而 CI 上永远比本机忙。
 *
 * 试过的省法：把逐页的硬等换成条件等待只能省下 8 秒，占不到总时长的 15%，
 * 压不下来。截图是这套走查的产物（存到 test-results/走查/ 给人翻），不能删。
 * 所以结论是给它应得的时间，而不是把它改瘦。
 */
