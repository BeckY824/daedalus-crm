...
import { buildProposal, describeProposal, summarizeApplied } from "@/lib/agent/proposals";
import { dayjs } from "@/lib/utils";

const 客户 = { id: "c1", name: "陈同学" };
const 建 = (kind: Parameters<typeof buildProposal>[1], args: Record<string, unknown>) => buildProposal("p1", kind, 客户, args);
...
describe("记一条跟进的提议", () => {
  ...
  it("内容太短不给记——空记录比没记录更糟", () => {
    expect(建("add_followup", { type: "PHONE", content: "嗯", reason: "口述" }).ok).toBe(false);
  });
