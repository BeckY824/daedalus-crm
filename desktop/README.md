...
Windows x64 的构建、数据目录与更新协议见 [Windows 版](../docs/Windows版.md)。Windows 与 Mac 共用本地 CRM、云端账号和连接服务器模式。

一个 Electron 应用，两种模式，同一个安装包：

- **本地** —— 整套 CRM 跑在这台机器上，数据是一个本地 SQLite 文件。装完就能用，
  不需要自己部署服务器；首次进入需要注册并登录云端账号。新装默认这个。
- **服务器** —— 连一台已经部署好的实例，团队共用一份数据。从旧版本升上来的保持这个模式。

切到服务器在设置页「桌面端 → 连接服务器」，切回来在应用菜单「改用本机数据」。切到服务器不会动本地的数据，切回来还在。
...
要先登录云端账号才能进（第一次打开就是登录页，注册走网页）。登录之后，桌面端拿到一枚长期设备令牌，**直接当模型接口的
...
- 免费次数：第一次登录送 30，余额不足 30 时每天再送 3。设置页能看还剩多少。
  按提问扣：同一个问题的几步请求带同一个 `X-Question-Id`，网关只扣第一次；上游 5xx / 429 / 408 / 连不上的那次退回。
...
（或 CI 的 x64 runner）上再打一次。Windows 版在 Windows x64 构建机上执行 `npm run dist:win`。
