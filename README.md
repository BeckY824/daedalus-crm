...
不想自己部署，用邮箱在 [app.ai-daedalus.com/signup](https://app.ai-daedalus.com/signup) 一分钟开出自己的工作区——邮箱、验证码、密码、团队名，没有别的门槛。7 天全功能，到期只读、数据不删。AI 对话注册送 30 次，用了之后每天再送 3 次。想先随便点点，[在线演示](https://app.ai-daedalus.com/demo)不用注册也不用码。
...
同一份代码，`MULTI_TENANT=1` 打开多租户：一个工作区一个 SQLite 文件，物理隔离；邮箱验证码注册、7 天试用、到期只读、订阅与运营台、AI 免费次数账本（注册送 30、每天送 3）。自部署版不开这个开关，一行相关代码都不会执行。
...
| 测试 | vitest（438 单测）· Playwright（58 自部署 + 9 托管版 e2e）· CI 全绿才能合并 |
| 交付 | Docker 多架构镜像（GHCR）· Electron 桌面端（Mac / Windows）· Caddy 自动 HTTPS |
