...
- ✅ 三栏界面：中间一栏永远是「当下该看的」——首页是今天要跟的人和待办
...
第一次打开会先弹登录窗，**用邮箱注册一个免费账号**（登录窗里就能点去注册、也能直接改密码）；
账号只用来记 AI 次数，数据不上传。

AI 有两条路：用账号自带的免费次数（注册送 30 次，每天登录再补 3 次），
或者在「设置管理 → AI 接入」填自己的模型 API Key——
...
左边 76px 只放图标，鼠标停住才出名字；中间一栏放这个模块**当下最该看的东西**——首页是「今天」（今天要跟的人和到期的待办，逾期标红，勾一下就完成），学员页是最近跟进的 50 位（点一行右边就出档案），设置页是五个设置项；右边才是正文。桌面端再把系统标题栏去掉，红黄绿钮嵌进图标栏顶上。

（和下面「记录页」那三栏不是一回事：那是学员详情页**内部**的分栏。）
...
| 记录页 | 档案行内编辑、时间线、速记解析、AI 常驻 | ✅ 已发布 |
| 三栏壳 | 图标栏 / 当下栏 / 正文，首页「今天」，桌面端嵌入窗口按钮 | ✅ 已发布 |
