      ...
      if (f.kind === "enum") {
        ...
        if (!hit) {
          /*
            ...
            库里这一列本来就是自由文本，那张下拉表只是界面上的建议；
            而人手上那份表里的职位千奇百怪，丢掉等于档案缺一块，
            他当时只看到一行灰字，事后才发现。

            封闭的那两个（跟进状态 / 决策状态）照旧用默认值——它们被盯盘、
            建议卡、报表分组吃着，塞一个没人认识的值进去会静悄悄打乱统计。
          */
          if (f.开放) {
            值[字段] = v;
            问题.push({ 列, 字段, 原值: v, 说法: `「${v}」不在${f.label}的选项里，会按原样导入`, 严重: "照收" });
            return;
          }
